...
const createPreviewEntry = (code: string) => `
...
export class SimpleCodeGeneratorAgent extends Agent<Env, CodeGenState> {
			...
			for (let i = 0; i < reviewCycles; i++) {
				...
				if (issuesFound) {
					...
					for (const fileToFix of reviewResult.filesToFix) {
						...
						const fileToRegenerate = this.fileManager.getGeneratedFile(
							fileToFix.filePath,
						);
						if (!fileToRegenerate) {
							this.logger().warn(
						...
						promises.push(
							this.regenerateFile(fileToRegenerate, fileToFix.issues, 0),
						);
					}
	...
	private migrateStateIfNeeded(): void {
			...
			if (migratedConversationMessages.length !== originalCount) {
				this.logger().info('Fixed conversation message exponential bloat', {
					...
					internalMemosRemoved:
						uniqueMessages.length - migratedConversationMessages.length,
				});
				needsMigration = true;
			}
		}
		...
		const stateHasProjectUpdatesAccumulator =
			'projectUpdatesAccumulator' in (this.state as any);
	...
	private async createNewPreview(): Promise<PreviewType | null> {
		...
		const createResponse = await this.getSandboxServiceClient().createInstance(
			templateName,
			`v1-${projectName}`,
			webhookUrl,
		);
	...
	private async executeDeployment(
		...
		try {
			const { sandboxInstanceId, previewURL, tunnelURL } =
				await this.ensurePreviewExists(redeploy);

			let filesToWrite;

			if (this.state.blueprint.frameworks?.includes('react-native')) {
				this.logger().info(
					'React Native project detected, transforming for web preview.',
				);
				const appFile = this.fileManager.getGeneratedFile('src/App.tsx');
				if (!appFile) {
					throw new Error(
						'Could not find src/App.tsx for React Native preview.',
					);
				}

				const entryPoint = createPreviewEntry(appFile.fileContents);
				...
				if (result.outputFiles.length === 0) {
					throw new Error('esbuild bundling failed for React Native preview.');
				}

				const js = result.outputFiles[0].text;
				const html = htmlTemplate(js);

				filesToWrite = [{ filePath: 'index.html', fileContents: html }];
				this.logger().info(
					'Successfully transformed React Native component to a single index.html file.',
				);
			} else {
				// Deploy files
				filesToWrite =
			...
			if (filesToWrite.length > 0) {
				const writeResponse = await this.getSandboxServiceClient().writeFiles(
					sandboxInstanceId,
					filesToWrite,
					commitMessage,
				);
	...
	async pushToGitHub(options: GitHubPushRequest): Promise<GitHubExportResult> {
		try {
			...
			const exportResult = await this.getSandboxServiceClient().pushToGitHub(
				this.state.sandboxInstanceId!,
				options,
			);
